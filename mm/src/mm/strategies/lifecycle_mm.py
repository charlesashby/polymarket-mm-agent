"""
Enhanced market-maker with comprehensive order lifecycle management (Milestone #6).

This strategy builds on InventoryAwareMM by adding:
1. Track open orders per instrument ✓ (from InventoryAwareMM)
2. Cancel stale orders on new market data ✓ (from InventoryAwareMM)
3. Implement order timeout mechanism ✓ (NEW)
4. Replace orders when quotes need updating ✓ (from InventoryAwareMM)
5. Handle partial fills correctly ✓ (from InventoryAwareMM)
6. Log all order submissions and fills ✓ (from InventoryAwareMM)
7. Verify no duplicate orders submitted ✓ (NEW - improved)
8. Test order replacement is atomic ✓ (from InventoryAwareMM)

New features in this strategy:
- Order age tracking with timeout mechanism
- Minimum requote interval to prevent excessive message overhead
- Better duplicate order prevention
- Enhanced logging for order lifecycle events
"""
...
class LifecycleMM(BaseStrategy):
    """
    Market-maker with comprehensive order lifecycle management.

    Parameters
    ----------
    spread : float
        Base half-spread in decimal (e.g., 0.01 for 1 cent on each side)
    ...
    min_requote_interval_secs : float
        Minimum time between requotes for same instrument (default: 0.0 = disabled)
    order_timeout_secs : float
        Maximum age for open orders before forced cancel/replace (default: 30.0 seconds)
    verbose : bool
    ...
    def __init__(
        ...
        min_requote_interval_secs: float = 0.0,
        order_timeout_secs: float = 30.0,
        verbose: bool = False,
        **kwargs: Any
    ) -> None:
        """
        Initialize the lifecycle-aware market-maker strategy.
        ...
        min_requote_interval_secs : float, default 0.0
            Minimum time between requotes (0 = disabled, allows requote on every market data update)
        order_timeout_secs : float, default 30.0
            Maximum age for orders before forced cancel/replace
        ...
        **kwargs
        ...
        self.min_requote_interval_secs = float(min_requote_interval_secs)
        self.order_timeout_secs = float(order_timeout_secs)
        self.verbose = bool(verbose)

        # Track open orders per instrument with submission timestamps
        self._open_orders: Dict[InstrumentId, Dict[OrderSide, LimitOrder]] = defaultdict(dict)
        self._order_submit_times: Dict[InstrumentId, Dict[OrderSide, float]] = defaultdict(dict)

        # Track last requote time per instrument (for rate limiting)
        self._last_requote_time: Dict[InstrumentId, float] = {}
    ...
    def on_start(self) -> None:
        ...
        print(f"[LifecycleMM] Strategy started")
        print(f"[LifecycleMM]   Base spread: {self.spread}")
        print(f"[LifecycleMM]   Quote notional: {self.quote_notional}")
        print(f"[LifecycleMM]   Position limit: {self.position_limit}")
        print(f"[LifecycleMM]   Skew coefficient: {self.skew_coefficient}")
        print(f"[LifecycleMM]   Spread widening: {self.spread_widening}")
        print(f"[LifecycleMM]   Min requote interval: {self.min_requote_interval_secs}s")
        print(f"[LifecycleMM]   Order timeout: {self.order_timeout_secs}s")
        print(f"[LifecycleMM]   Verbose: {self.verbose}")
        print(f"[LifecycleMM]   Tracking {len(self._slugs)} slug pairs")

    def _handle_quote_tick(self, tick: QuoteTick) -> None:
        """
        Handle quote tick updates.

        Updates direct BBO and triggers quote update.
        """
    ...
    def _handle_trade_tick(self, tick: TradeTick) -> None:
        """Handle trade tick updates (not used for quoting, but logged)."""
        self._data_processed += 1
    ...
    def _handle_order_book_depth10(self, depth: OrderBookDepth10) -> None:
        ...
        if depth.bids and depth.asks:
            bid_price = float(depth.bids[0].price) if depth.bids[0].price else 0.0
            ask_price = float(depth.asks[0].price) if depth.asks[0].price else 0.0
        ...
        self._check_order_timeouts(inst_id)
    ...
    def _should_requote(self, inst_id: InstrumentId) -> bool:
        """
        Check if we should requote based on minimum interval throttle.

        Returns True if enough time has passed since last requote.
        """
        current_time = time.time()
        last_requote = self._last_requote_time.get(inst_id, 0.0)

        if current_time - last_requote < self.min_requote_interval_secs:
            self._requote_throttled += 1
            return False

        return True

    def _check_order_timeouts(self, inst_id: InstrumentId) -> None:
        """
        Check if any open orders have exceeded timeout threshold.

        Cancel orders that are too old to ensure fresh quotes.
        """
        current_time = time.time()
        open_orders = self._open_orders.get(inst_id, {})
        submit_times = self._order_submit_times.get(inst_id, {})

        for side, order in list(open_orders.items()):
            if not order or not order.is_open:
                continue

            submit_time = submit_times.get(side, current_time)
            age = current_time - submit_time

            if age > self.order_timeout_secs:
                if self.verbose:
                    print(f"[LifecycleMM] Order timeout: {order.client_order_id} age={age:.1f}s")
                self.cancel_order(order)
                self._order_timeouts += 1
                # Remove from tracking
                if side in open_orders:
                    del open_orders[side]
                if side in submit_times:
                    del submit_times[side]

    def _has_duplicate_order(self, inst_id: InstrumentId, side: OrderSide) -> bool:
        """
        Check if we already have an open order for this instrument and side.

        Returns True if duplicate exists (prevents duplicate submission).
        """
        open_orders = self._open_orders.get(inst_id, {})
        existing_order = open_orders.get(side)

        if existing_order and existing_order.is_open:
    ...
    def _update_quotes(self, inst_id: InstrumentId) -> None:
        ...
        Logic:
        1. Check requote throttle (min interval)
        2. Compute effective BBO (direct + implied from complement)
        3. Calculate mid price from effective BBO
        4. Calculate inventory skew adjustment
        5. Calculate spread widening from position size
        6. Calculate quote prices with skew and widened spread
        7. Check position limits before quoting each side
        8. Enforce price bounds [0.01, 0.99]
        9. Check for duplicate orders
        10. Cancel stale orders
        11. Submit new orders with timestamp tracking
        """
        # Step 1: Check requote throttle
        if not self._should_requote(inst_id):
            return
        ...
        self._order_submit_times[inst_id] = {}
        ...
        # Note: We don't need duplicate check since we just canceled all existing orders above
        if can_buy:
            ...
            if buy_order:
                ...
                self._order_submit_times[inst_id][OrderSide.BUY] = current_time
                self._quotes_sent += 1
        ...
        if can_sell:
            ...
            if sell_order:
                ...
                self._order_submit_times[inst_id][OrderSide.SELL] = current_time
                self._quotes_sent += 1

        # Update last requote time
        self._last_requote_time[inst_id] = current_time

        # Verbose logging
        if self.verbose and self._quotes_sent % 100 == 0:
            print(f"[LifecycleMM] Quotes={self._quotes_sent} fills={self._fills} " +
                  f"throttled={self._requote_throttled} timeouts={self._order_timeouts}")
    ...
    def on_order_filled(self, event) -> None:
        ...
        if inst_id in self._open_orders:
            for tracked_side, tracked_order in list(self._open_orders[inst_id].items()):
                if tracked_order and tracked_order.client_order_id == client_order_id:
                    ...
                    if tracked_side in self._order_submit_times.get(inst_id, {}):
                        del self._order_submit_times[inst_id][tracked_side]
                    break
        ...
        if self.verbose:
            ...
            print(f"[LifecycleMM] FILL #{self._fills}: {inst_id} {side_str} " +
    ...
    def _on_stop(self) -> None:
        ...
        print(f"[LifecycleMM] Strategy stopped")
        print(f"[LifecycleMM] Statistics:")
        print(f"[LifecycleMM]   Data processed: {self._data_processed}")
        print(f"[LifecycleMM]   Quotes sent: {self._quotes_sent}")
        print(f"[LifecycleMM]   Fills: {self._fills}")
        print(f"[LifecycleMM]   Position limit hits: {self._position_limit_hits}")
        print(f"[LifecycleMM]   Skew adjustments: {self._skew_adjustments}")
        print(f"[LifecycleMM]   Effective BBO used: {self._effective_used} times")
        print(f"[LifecycleMM]   Requote throttled: {self._requote_throttled}")
        print(f"[LifecycleMM]   Order timeouts: {self._order_timeouts}")
        print(f"[LifecycleMM]   Duplicates prevented: {self._duplicate_prevented}")

        # Print final positions
        print(f"[LifecycleMM] Final positions:")
        for inst_id in sorted(self._positions.keys()):
            position = self._positions[inst_id]
            if abs(position) > 0.001:
                cost_basis = self._cost_basis[inst_id]
                print(f"[LifecycleMM]   {inst_id}: pos={position:.0f} basis={cost_basis:.3f}")
